...
{
    class DataLoaderGLAC_old : DataLoader
    {
        string _folderPath; //test
        bool _hasHeaders;
        private List<IFixedWidthField> _fields = new List<IFixedWidthField>();

        public override string SqlTableName => "GIS.PoliciesGLAC_old";

        public DataLoaderGLAC_old()
        {
        ...
        public override void LoadToSql()
        {
            ...
            foreach (string file in files)
            {
                ...
                DateTime? date = Extensions.Extensions.GetFirstDateFromString(file, @"\d{6}", "yyyyMM");
                if (date != null)
                {
                    DateTime asOfDate = (DateTime)date;
                    asOfDate = new DateTime(asOfDate.Year, asOfDate.Month, DateTime.DaysInMonth(asOfDate.Year, asOfDate.Month));
